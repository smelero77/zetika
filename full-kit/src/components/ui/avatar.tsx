...
export function AvatarImage({
  ...
  src,
  ...props
}: ComponentProps<typeof AvatarPrimitive.Image> & { src?: string }) {
  const safeSrc = src && src.trim() !== "" ? src : "/images/avatars/male-01.svg"
  return (
...
export function AvatarStack({
  ...
  return (
    <div className={cn("flex", className)} {...props}>
      ...
      {remainingCount > 0 && onMoreButtonClick && (
        <button
          type="button"
          className={cn(
            avatarStackVariants({ size }),
            avatarClassName,
            "bg-muted border-2 border-background text-xs font-semibold"
          )}
