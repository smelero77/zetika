...
/**
 * Sincroniza el catálogo de Fondos
 */
async function syncFondos(jobName: string, runId: string) {
  const catalogName = 'Fondos';
  const endpoint = '/fondos';
  ...
  try {
    ...
    const items: { id: number; nombre: string }[] =
    ...
    for (const item of items) {
      ...
      try {
        await upsertWithRetry(dbETL.fondo, {
        ...
        batchLogger.progress(batchJobName, `Procesando ${catalogName}`, { 
          ...itemMeta, 
          itemData: { id: item.id, nombre: item.nombre },
          durationMs: duration 
        }, 50);
      } catch (e: unknown) {
        errors++;
...
export async function POST(req: Request) {
  ...
  const jobName = 'sync-fondos';
  const runId = crypto.randomUUID();
  metrics.increment('etl.jobs.invoked');
  const startTime = Date.now();
  logger.info('Job Fondos Invocado', { jobName, runId });

  try {
    const stats = await syncFondos(jobName, runId);

    const durationMs = Date.now() - startTime;
    metrics.histogram('etl.jobs.total_duration_ms', durationMs);
    logger.info('Job Fondos Completado', { jobName, runId, stats, durationMs });
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    const errorObj = error instanceof Error ? error : new Error(errorMessage);
    logger.error('Job Fondos Falló Catástroficamente', errorObj, { jobName, runId, durationMs });
